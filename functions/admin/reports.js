...
import { badge, drawer, esc, errorPage, field, guard, jsonBlock, layout, page, RE_FILTER, sourceBadge, sql, table } from './_layout.js'

const PAGE_SIZE = 50
const FIELDS = ['id', 'sig', 'category', 'shell', 'plugin', 'plugin_ver', 'code', 'source', 'turns', 'created_at']
...
export async function onRequestGet(context) {
  ...
  const q = Object.fromEntries(
    ['sig', 'category', 'shell', 'plugin', 'source']
  ...
  try {
    const [[count], rows] = await Promise.all([
      ...
      sql(env, `SELECT id, sig, category, shell, plugin, plugin_ver, code, source, turns, created_at
    ...
    const bodyRows = rows.map((r) => `<tr class="hover:bg-indigo-50 ${id === Number(r[0]) ? 'bg-indigo-50' : ''}">
      ...
      <td class="px-4 py-2.5">${sourceBadge(r[7])}</td>
      <td class="px-4 py-2.5 text-right">${r[8] ?? ''}</td>
      <td class="px-4 py-2.5 text-slate-400">${esc(String(r[9]).slice(0, 19))}</td></tr>`).join('')
    ...
    return page(layout({
      ...
      <form method="get" action="/admin/reports" class="mb-5 flex flex-wrap items-center gap-2">
        ${input('sig', 'sig', 20)}${input('category', 'category', 12)}${input('shell', 'shell', 10)}${input('plugin', 'plugin', 14)}${input('source', 'source (organic/seed)', 18)}
      ...
      <div class="flex items-start gap-6">
        <div class="min-w-0 flex-1">
          ${table(['id', 'sig', 'category', 'shell', 'plugin', 'ver', 'code', '来源', 'turns', '时间'], bodyRows)}
